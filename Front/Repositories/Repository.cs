...
namespace Front.Repositories
{
    public class Repository : IRepository
    {
        ...
        public async Task<HttpResponseWrapper<object>> PostAsync(string url)
        {
        ...
        public async Task<HttpResponseWrapper<T>> PutAsync<T>(string url)
        {
            var responseHttp = await _httpClient.PutAsync(url, null);
            if (responseHttp.IsSuccessStatusCode)
            {
                var response = await UnserializeAnswerAsync<T>(responseHttp);
                return new HttpResponseWrapper<T>(response, false, responseHttp);
